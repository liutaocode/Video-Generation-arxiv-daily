...
def get_daily_papers(topic,query="slam", max_results=2):
    ...
    # Use separate searches to avoid pagination issues
    # Split the max_results into chunks of 100 or less
    papers_collected = 0
    batch_size = min(100, max_results)
    num_batches = (max_results + batch_size - 1) // batch_size
    
    logging.info(f"Collecting {max_results} papers in {num_batches} batches of {batch_size}")
    
    for batch_idx in range(num_batches):
        remaining = max_results - papers_collected
        current_batch_size = min(batch_size, remaining)
        
        if current_batch_size <= 0:
            break
            
        logging.info(f"Batch {batch_idx + 1}/{num_batches}: Fetching {current_batch_size} papers")
        ...
        try:
            for result in client.results(search):
                ...
                logging.info(f"[{papers_collected}/{max_results}] Time = {update_time} title = {paper_title} author = {paper_first_author}")
            ...
            logging.info(f"Batch {batch_idx + 1} returned empty page, skipping to next batch")
            pass
        except Exception as e:
            logging.error(f"Error fetching papers in batch {batch_idx + 1}: {e}")
            pass
    ...
    logging.info(f"Total papers collected: {papers_collected}")
    data = {topic:content}
